        ...
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers(
                                ...
                                new AntPathRequestMatcher("/*.json"), // JSON 파일 (예: manifest.json)
                                new AntPathRequestMatcher("/*.ico"), // favicon.ico
                                new AntPathRequestMatcher("/*.css"), // CSS 파일
                                new AntPathRequestMatcher("/static/**"), // 정적 리소스 (CRA 스타일)
                                new AntPathRequestMatcher("/assets/**"), // **Vite 빌드 결과물 (가장 중요)**
                        ...
                        .requestMatchers(
                                new AntPathRequestMatcher("/users/**"), // 사용자 관련 비인증 API (ex: 회원가입, 이메일 인증)
                        ...
                        .requestMatchers(
                                new AntPathRequestMatcher("/login-process"), // 폼 로그인 POST 처리
                                new AntPathRequestMatcher("/logout"), // 로그아웃 POST/GET 처리
                                new AntPathRequestMatcher("/oauth2/**"), // OAuth2 로그인 시작
                                new AntPathRequestMatcher("/login/oauth2/code/**") // OAuth2 콜백
                        ).permitAll()
                        .requestMatchers(
                                new AntPathRequestMatcher("/login"), // React 라우터의 /login 경로
                                new AntPathRequestMatcher("/signup"),
                                new AntPathRequestMatcher("/items"), // 이전에 에러가 났던 /items
                                // ... 여기에 React 라우터가 처리하는 모든 최상위 경로를 추가

                                // 더 일반적인 SPA 클라이언트 라우팅 처리:
                                // /api/로 시작하지 않는 모든 경로 (단일 경로 세그먼트)를 permitAll
                                new AntPathRequestMatcher("/{path:[^\\.]*}"), // 예: /test, /my-page (파일 확장자 없는 경로)
                                // /api/로 시작하지 않는 모든 경로 (다중 경로 세그먼트)를 permitAll
                                // 이 패턴은 `/api/`로 시작하는 실제 백엔드 API와 충돌하지 않도록 주의해야 합니다.
                                new AntPathRequestMatcher("/{path:^(?!api$).*$}/**") // 예: /my-page/sub-page
                        ).permitAll()
                ...
                .oauth2Login(oauth2 -> oauth2
                        ...
                        .successHandler((request, response, authentication) -> {
                            response.sendRedirect("http://localhost:5173/"); // 소셜 로그인 성공 시 프론트엔드 메인 페이지로 이동
                        })
                        .failureHandler((request, response, exception) -> {
                            response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
